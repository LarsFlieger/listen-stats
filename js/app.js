...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (downloadHtmlBtn) {
    downloadHtmlBtn.onclick = async () => {
      try {
        const cssText = await fetch('css/style.css').then(r => r.text());
        const resultsHTML = document.getElementById('results').innerHTML;
        const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>ListenStats Share</title><style>${cssText}</style></head><body>${resultsHTML}</body></html>`;
        ...
        console.error('Failed to generate HTML file:', err);
  ...
  if (downloadPdfBtn) {
    downloadPdfBtn.onclick = async () => {
      try {
        ...
        printWindow.document.write(`<html><head><title>ListenStats</title><style>${cssText}</style></head><body>${resultsHTML}</body></html>`);
        printWindow.document.close();
        printWindow.focus();
        printWindow.print();
      } catch (err) {
        console.error('Failed to generate PDF:', err);
        alert('Unable to generate PDF file.');
      }
    };
  }
});
...
// Generate a shareable URL and copy it to the clipboard
